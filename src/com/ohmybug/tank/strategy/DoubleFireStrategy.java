...
public class DoubleFireStrategy implements FireStrategy {
    ...
    public void fire(Player player) {
        ...
        if (player.getDirection() == Direction.D || player.getDirection() == Direction.U) {
            TankFrame.INSTANCE.getGM().add(new Bullet(bX, bY, Direction.D, player.getGroup()));
            TankFrame.INSTANCE.getGM().add(new Bullet(bX, bY, Direction.U, player.getGroup()));
        }
        if (player.getDirection() == Direction.L || player.getDirection() == Direction.R) {
            TankFrame.INSTANCE.getGM().add(new Bullet(bX, bY, Direction.L, player.getGroup()));
            TankFrame.INSTANCE.getGM().add(new Bullet(bX, bY, Direction.R, player.getGroup()));
        }
    }
}
